...
contract VolcanoCoinExtropy is ERC20("Volcano Coin", "VLC"), Ownable {
    uint256 paymentID;
        ...
        returns (bool)
    {
        ...
        RecordIDs[newID] = RecordIndex({
            user: _msgSender(),
            arrayIndex: payments[_msgSender()].length - 1
        });
        return true;
    }

    function findPaymentDetailsIndex(uint256 _id)
        internal
        view
    ...
    function updatePaymentDetails(
        ...
        require(
            user == _msgSender() || _msgSender() == administrator,
            "Only Owner or administrator"
        );
        string memory comment = _comment;
        if (_msgSender() == administrator) {
            string memory adminAddress = Strings.toHexString(
                uint256(uint160(administrator))
            );
            comment = string(
                abi.encodePacked(_comment, " updated by ", adminAddress)
            );
        }
